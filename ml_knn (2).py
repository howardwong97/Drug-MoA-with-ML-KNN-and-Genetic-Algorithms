...
def oof_probas(X, y, v, weight, n_folds=5, random_state=42, n_neighbors=1000):
    # First scale inputs
    ...
    for fold in range(n_folds):
        ...
        model.fit((X_scaled[fold_idx][:, v]) * weight.T, y.iloc[fold_idx].values[:, 1:])

        pp = model.predict_proba((X_scaled[pp_fold][:, v]) * weight.T)
...
class FeatureScaling:
    def _init_(self, final_features, n_folds=5, random_state=42, n_neighbors=1000):
    ...
    def fit(self, X, y, generations=20, solution_per_population=100,
            mutation_rate=0.05):

        population_size = (solution_per_population, X.shape[1])
        scales = np.random.uniform(0, 1, population_size) ** 2.0
        fitness_scores = np.zeros(solution_per_population)

        for generation in range(generations):
            print('=========== GENERATION', generation + 1, '=========== \n', 'Computing log loss scores...')
            scales = np.array([scales[i] for i in np.argsort(fitness_scores)])

            for k in range(solution_per_population):
                parent1 = k // n_parents
                parent2 = k % n_parents
                TMP = np.random.randint(0, 2, X.shape[1])
                scales[k,] = TMP * scales[parent1, :-1] + (1 - TMP) * scales[parent2, :-1]
                x = np.random.uniform(0, 1, X.shape[1])
                IDX = np.where(x < mutation_rate)[0]
                scales[k, IDX] = np.random.uniform(0, 1, len(IDX)) ** 2.0

                scales[k, 0] = 100
                scales[k, 1] = 12 / 2
                scales[k, 2] = 5 / 2

                for final_feature_subset in self.final_features:
                    prediction_proba = oof_probas(X, y, final_feature_subset, weight=scales[k, :], n_folds=n_folds,
                                                  random_state=random_state,
                                                  n_neighbors=n_neighbors)
                    fitness_scores[k] = (log_loss(y.iloc[:, 1:].values.flatten(), prediction_proba.flatten()))
